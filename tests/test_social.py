...
def test_post_calls_send_image(tmp_path):
    ...
    chart_path.write_bytes(b"fake png")
    story = {
        "social_text": "Draisaitl is hot.",
        "headline": "Hat trick alert",
        "chart": "chart.png",
    }
    ...
    with (
        patch("scripts.social.Client", return_value=mock_client),
        patch.dict(
            "os.environ", {"BLUESKY_HANDLE": "user.bsky.social", "BLUESKY_APP_PASSWORD": "pass"}
        ),
    ):
    ...
    assert "Draisaitl is hot." in call_kwargs.kwargs.get(
