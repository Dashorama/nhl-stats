...
def test_reconcile_ground_truth():
    ...
    assert {(owners[k.team], normalize(k.player), k.first_year, k.traded) for k in result} == {
        (owners[r["Team Name"]], normalize(reference_names.get(r["Player Name"], r["Player Name"])),
         int(r["First Year Kept"]), int(r["Traded?"])) for r in expected
    }
...
def test_board_spelling_and_original_sheet_order_replace_csv_cosmetics():
    ...
    canonical = {p["player"] for t, picks in board.items() if t != "raw_full_draft"
                 for p in picks if p.get("keeper") is True}
...
def small_board_snapshot():
    board = {"A": [{"player": p, "keeper": True} for p in
                    ["New Alpha", "Matthew Tkachuk", "New Beta"]],
             "B": [{"player": p, "keeper": True} for p in
...
def test_global_contract_carry_and_new_append_order_with_move_warning():
    ...
    assert result == [Keeper("A", "Matthew Tkachuk", 2022, 2),
                      Keeper("A", "New Alpha", 2026, 0), Keeper("A", "New Beta", 2026, 0),
                      Keeper("B", "Other Two", 2023, 0), Keeper("B", "Other One", 2026, 0),
                      Keeper("B", "Other Three", 2026, 0)]
    assert warnings == ["possible unrecorded trade: Matthew Tkachuk sheet-team B -> "
...
def test_cumulative_trade_from_existing_count_and_already_reflected_trade():
    ...
    assert scanTrades(result, [trade], season=2026, state=state,
                      known_teams=["A", "B"]) == (result, state)
    assert scanTrades(result, [trade], season=2026,
                      known_teams=["A", "B"])[0] == result
